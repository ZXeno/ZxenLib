...
public static partial class CollisionUtils
{
        ...
        in Transform xfB)
    {
        ...
        // 在边缘形状的外框处理圆形
        Vector2 Q = MathUtils.MulT(xfA, MathUtils.Mul(xfB, circleB.Position));
        ...
        ContactFeature cf = new ContactFeature {IndexB = 0, TypeB = (byte)ContactFeature.FeatureType.Vertex};
        ...
        if (v <= 0.0f)
        {
            ...
            ref ManifoldPoint point = ref manifold.Points.Value0;
            point.Id.Key = 0;
    ...
    static EPAxis ComputeEdgeSeparation(in TempPolygon polygonB, in Vector2 v1, Vector2 normal1)
    {
        ...
        Vector2[]? axes = new[] {normal1, -normal1};
        ...
        in Transform xfB)
    {
        ...
        {
            float separation = Vector2.Dot(refFace.Normal, clipPoints2[i].Vector - refFace.V1);
