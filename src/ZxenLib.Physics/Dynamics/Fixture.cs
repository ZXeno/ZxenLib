...
public class Fixture : IDisposable
{
    ...
    internal void CreateProxies(in BroadPhase broadPhase, in Transform xf)
    {
        ...
        for (int i = 0; i < this.ProxyCount; ++i)
        {
            ...
            this.Shape.ComputeAABB(out proxy.AABB, xf, i);
            proxy.Fixture = this;
            proxy.ChildIndex = i;
    ...
    public void Refilter()
    {
        ...
        LinkedListNode<ContactEdge>? node = this.Body.ContactEdges.First;
        while (node != null)
        {
            Contact? contact = node.Value.Contact;
            node = node.Next;
        ...
        if (this.Body._world == null)
        {
            return;
        }

        // Touch each proxy so that new pairs may be created
        BroadPhase? broadPhase = this.Body._world.ContactManager.BroadPhase;
        for (int i = 0; i < this.ProxyCount; ++i)
        {
    ...
    {
        return this.Shape.RayCast(out output, input, this.Body.GetTransform(), childIndex);
    }
    ...
    {
        this.Shape.ComputeMass(out massData, this.Density);
    }
    ...
    internal void Synchronize(in BroadPhase broadPhase, in Transform transform1, in Transform transform2)
    {
        ...
        for (int i = 0; i < this.ProxyCount; ++i)
        {
            ...
            this.Shape.ComputeAABB(out AABB aabb1, transform1, proxy.ChildIndex);
            this.Shape.ComputeAABB(out AABB aabb2, transform2, proxy.ChildIndex);
    ...
    public void Dispose()
    {
        Array.Clear(this.Proxies, 0, this.Proxies.Length);
        this.Proxies = null;
        this.Body = null;
    }
}
...
public struct Filter
{
    ...
    {
        get => this._categoryBits.GetValueOrDefault(0x0001);
        set => this._categoryBits = value;
    }
    ...
    {
        get => this._maskBits.GetValueOrDefault(0xFFFF);
        set => this._maskBits = value;
    }
...
public struct FixtureDef
{
    ...
    {
        get => this._friction.GetValueOrDefault(0.2f);
    ...
    public float RestitutionThreshold
    {
        get => this._restitutionThreshold.GetValueOrDefault(1.0f * Settings.LengthUnitsPerMeter);
        set => this._restitutionThreshold = value;
    }
